...
Resultant_a = sqrt(V^2+Eb^2-2*V*Eb*cosd(alpha));
%calculating resultant angle
Angle_resultant_plot=atand(Eb*sind(alpha)/(V-Eb*cosd(alpha)));

if Eb < V-epsilon
    Angle_resultant_used=90+Angle_resultant_plot
    
elseif Eb > V+epsilon 
    Angle_resultant_used=270-Angle_resultant_plot;
    
else
    Angle_resultant_used=180

end

Resultant=Resultant_a*exp(1j*deg2rad(Angle_resultant_used));
